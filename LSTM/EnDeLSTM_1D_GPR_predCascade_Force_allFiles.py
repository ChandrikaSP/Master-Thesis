...
data_raw = open("fy1_1.txt").read().split() #"amazon.txt" OnlyDY
data_raw = np.array(data_raw).astype('float64')
data_tr_1_2 = open("fy1_2.txt").read().split() #"amazon.txt" OnlyDY
data_tr_1_2 = np.array(data_tr_1_2).astype('float64')
data_tr_1_3 = open("fy1_3.txt").read().split() #"amazon.txt" OnlyDY
data_tr_1_3 = np.array(data_tr_1_3).astype('float64')

print(np.shape(data_raw))
data_raw=data_raw.reshape((np.shape(data_raw)[0]),1)
scaler = MinMaxScaler()
scaler.fit(data_raw.reshape(-1,1))

# GPR=np.load('920_25_realz_1.npy')
GPR=np.load('fy_E1.1_75_realz.npy')
GPR=GPR.T
GPR_GT = np.load('fyEXACT_E1.1.npy')
GPR_GT = scaler.inverse_transform(GPR_GT)
GPR_GT=(GPR_GT[:,1]).reshape(-1,1)  #to get (920,2) 

GT_synthetic = open("fy1_1.txt").read().split()
...
#'Only_DY1_1.txt','Only_DY1_2.txt',   'fy1_1.txt','fy1_2.txt','fy1_3.txt',
csv_file_list =['fy1_1.txt','fy1_2.txt','fy1_3.txt','fy2_1.txt','fy2_2.txt','fy2_3.txt','fy3_1.txt','fy3_2.txt','fy3_3.txt','fy4_1.txt','fy4_2.txt','fy4_3.txt'] #,'fy5_1.txt','fy5_2.txt','fy5_3.txt'
list_of_dataframes = []
for filename in csv_file_list:
    ...
    GT_synthetic = open("fy1_1.txt").read().split()     #"fy2_1.txt""
    GT_synthetic = np.array(GT_synthetic).astype('float64')
    ...
    # in_data_raw, out_data_raw = split_sequences_new(data_raw, n_past, n_future, 1)
    # in_data_test, out_data_test = split_sequences_new(list_of_dataframes, n_past, n_future, 1)
    ...
    history=model.fit(GPR_in, GT_out, validation_split=0.33, epochs=200, verbose=2, shuffle=False)  #X, y, epochs=50  batch_size=256,   200
    ...
    # model.save('EnDeLSTM_1D_GPR_force1.1_1000e_75gpr_24nodes_final.h5')  #EnDeLSTM_1D_GPR_changedSSPredFun_1000e_75gpr_24nodes.h5
    
    # model = load_model('EnDeLSTM_1D_GPR_force1.1_1000e_75gpr_24nodes_final.h5')  #EnDeLSTM_1D_GPR_changedSSPredFun_1000e  EnDeLSTM_1D_GPR.h5 EnDeLSTM_1D_GPR_75samples_200e.h5 EnDeLSTM_1D_GPR_600e_75gpr_24nodes_final
    ...
    plt.ylabel('Force dY value (in millimeter [mm])')
    plt.title(filename)
    plt.legend()
    plt.grid()
    plt.show()
